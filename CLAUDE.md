...
- `session_service.append_chunk_result()` does an unlocked read-modify-write of `chunk_results` — under a real multi-worker pool (`CELERY_BROKER_URL` set, concurrency > 1), two chunks of the same session finishing near-simultaneously on different processes can lose one's update. A correct fix needs row-level locking or optimistic concurrency, untested without a real concurrent-load harness this sandbox doesn't have — see "No load testing has been done" above, same root cause.
