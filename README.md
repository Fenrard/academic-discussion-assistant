...
> **Noise-Aware Voice Recognition with Multilingual Code-Switching for Tagalog, English, and Hiligaynon: A Near Real-Time Academic Discussion Assistant Transcription Application**
...
## 4. Hugging Face account (required for diarization)
...
├── scripts/                       all six prototype scripts (section 6) + ws_smoke_test.py / v2_smoke_test.py
...
│                                  pubsub), utils/, data/ (section 8, below)
...
├── deployment/                    Dockerfile.api, Dockerfile.worker, docker-compose.yml, .env.example (section 11)
├── .github/workflows/ci.yml       runs the full test suite against real postgres+redis service containers
├── evaluation/                    wer.py, latency.py, teacher_id.py, sus.py, resources.py + reports/, plots/ output dirs (section 9)
├── ai/finetuning/                 LoRA/PEFT fine-tune + CTranslate2 convert scripts, not yet run (section 10)
...
## 7. Known issues / open items (scripts/)

- **Unresolved:** `record_test_audio.py` printed `Sample rate: 1600 Hz` in one test run instead of `16000 Hz` — never confirmed fixed. Worth re-checking before trusting downstream results.
...
- These six scripts stay CLI-only/file-path-based by design — see `backend/` (section 8) for the wired-up equivalent.
...
## 11. Production deployment (Docker + Postgres + Redis)
